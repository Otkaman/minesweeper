...
def reactor_efficiency(voltage, current, theoretical_max_power):
    ...
    perc = ((voltage * current) / theoretical_max_power) * 100
    if perc >= 80:
        return 'green'
    if 60 <= perc < 80:
        return 'orange'
...
def fail_safe(temperature, neutrons_produced_per_second, threshold):
    ...
    if temperature * neutrons_produced_per_second < 0.9 * threshold:
        return 'LOW'
    if 0.8 * threshold <= temperature * neutrons_produced_per_second <= 0.9 * threshold or \
    0.9 * threshold <= temperature * neutrons_produced_per_second <= 1.1 * threshold:
        return 'NORMAL'
    return 'DANGER'
